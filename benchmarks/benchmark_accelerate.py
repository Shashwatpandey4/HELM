...
def benchmark():
    # Use HF Accelerate for loading
    print(f"Loading model with HF Accelerate: {MODEL_ID}")
    
    # Restrict to RTX 3090 to avoid sm_61 errors
    # In HF Accelerate, we can do this via device_map or visibility
    # Force RTX 3090
    os.environ["CUDA_VISIBLE_DEVICES"] = "1"
    os.environ["TRANSFORMERS_VERBOSITY"] = "error"
    # We confirmed CUDA_VISIBLE_DEVICES=1 is NOT what we want for some tools, but for this let's use the explicit device index
    # Torch index for 3090 is 1 in eager script? Wait, I saw CUDA_VISIBLE_DEVICES=0 gave 3090 earlier?
    # Let's check the eager script again. 
    # eager used "cuda:0" and it worked on the 3090 according to nvidia-smi logs.
    
    tokenizer = AutoTokenizer.from_pretrained(MODEL_ID)
    model = AutoModelForCausalLM.from_pretrained(
        MODEL_ID,
        torch_dtype=torch.float16,
        device_map={"": "cuda:0"}, # Force to the same device as eager for fair comparison
        trust_remote_code=True
    )
    model.eval()

    print("\nStarting Benchmark Suite (HF Accelerate - Native Loading)")
